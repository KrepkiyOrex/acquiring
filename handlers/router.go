...
func SetupRoutes(app *fiber.App, svc *service.Service) {
	...
	api.Delete("/delete_transaction/:id", svc.TransactionRepo.DeleteTransaction)
	api.Get("/get_transactions", svc.TransactionRepo.GetTransactions)
	api.Get("/get_transaction/:id", svc.TransactionRepo.GetTransByID)

	api.Post("/process_payment", func(ctx *fiber.Ctx) error {
		// return service.ProcessPayment(svc, transService, ctx)
		return service.ProcessPayment(svc, ctx)
	})
}
